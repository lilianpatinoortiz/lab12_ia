...
class Node(object):
    def __init__(self):
        ...
        self.probabilities = None  #dictionary with p
...
# QX is a dictionary that has probabilities for different values.  this
...
def enumerateAll(varss, e,bn):
    ...
    auxNodes = deepcopy(varss)
    if len(aux) == 0: return 1.0
    if len(auxNodes) == 0: return 1.0
    Y = auxNodes.pop()
    if Y in e:
        val = Pr(Y,e[Y],e,bn) * enumerateAll(auxNodes,e,bn)
        # varss.append(Y)
        return val
    else:
        total = 0
        e[Y] = True
        total += Pr(Y,True,e,bn) * enumerateAll(auxNodes,e,bn)
        e[Y] = False
        total += Pr(Y,False,e,bn) * enumerateAll(auxNodes,e,bn)
        del e[Y]
        # varss.append(Y)
        return total
...
result =  enumerationAsk('Burglary',{'JohnCalls':True,'Earthquake':True},bn,varss)

print '%.7f'%result[True]